...
def calculate_demographic_data(print_data=True):
    ...
    higher_education_2 = df.loc[(df['education'].str.startswith('B')) | (df['education'].str.startswith('M')) | (df['education'].str.startswith('D'))]
    lower_education = df.loc[df['salary'] == '>50K']
    lower_education = len(lower_education) - len(higher_education)

    # percentage with salary >50K
    higher_education_rich = round(100 * (len(higher_education) / len(higher_education_2)),1)
    lower_education_rich = round(100 * (lower_education / (total_education - len(higher_education_2))), 1)
    ...
    new_df_2 = df['native-country'].value_counts()

    richest = (new_df/new_df_2).max()
    highest_earning_country = (new_df/new_df_2).sort_values(ascending=False).index[0]
